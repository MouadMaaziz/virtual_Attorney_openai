...
if __name__ == '__main__':
    ...
    text_block = '\n\n'.join(all_messages)
    chat_log = '<<BEGIN CLIENT INTAKE CHAT>>\n\n%s\n\n<<END CLIENT INTAKE CHAT>>' % text_block
    ...
    save_file('logs/log_%s_report.txt' % time(), report)
    print('\n\nLawyer\'s Report:\n\n%s' % report)
    ...
    conversation.append({'role': 'user', 'content': notes})
    form, tokens = chatbot(conversation)
    save_file('logs/log_%s_form.txt' % time(), form)
    print('\n\nform Requirements:\n\n%s' % form)

    ## SCENARIOS & OUTCOMES

    print('\n\nGenerating Scenarios and Tests')
    conversation = list()
    conversation.append({'role': 'system', 'content': open_file('system_05_scenario.md')})
    conversation.append({'role': 'user', 'content': notes})
    scenario, tokens = chatbot(conversation)
    save_file('logs/log_%s_scenario.txt' % time(), scenario)
